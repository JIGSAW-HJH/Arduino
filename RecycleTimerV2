...
void Nokia_Display_RTC_Time(int col, int row)
{
  ...
  display.setCursor(row, col);
  display.print(RTC_hours);
  display.write(":");
  display.print(RTC_minutes);
  display.write(":");
  display.print(RTC_seconds);
  display.display();//Update the display
}
...
void Nokia_Display_Volts(int col, int row)
{
  ...
  display.setCursor(row, col);
  display.print(BatteryVoltageLevel());
  display.write(" V");
  display.display();//Update the display
}
...
void Nokia_UpdateDateTimeSection(void)
{
  ...
  display.write("Set Date:");
  display.setCursor(11, 25);
  display.write("YYYY-MM-DD");
  display.display();//Update the display

  while(Done != true)//keep repeating until spaces to move = 0
  {
    //Write the new location for the ch:
    display.setCursor((11+chSpaces[spacesToMove]), 35);
    display.display();//Update the display
    display.write(24);//up arrow character
    ...
    if( currentKey != ' ')
    {
      ...
      display.setCursor((11+chSpaces[spacesToMove]), 25);
  ...
  display.setCursor(2, 15);
  display.write("Set Time:");
  Nokia_ClearLine(25,11,9);
  display.setCursor(11, 25);
  display.write("HH:MM:SS");//0,6, 18,24,  36,42
  display.display();//Update the display
  
  while(Done != true)//keep repeating until spaces to move = 0
  {
    //Write the new location for the ch:
    display.setCursor((11+chSpaces1[spacesToMove]), 35);
    display.display();//Update the display
    display.write(24);//up arrow character
    //Get the user input from the keypad:
    Keypad_Key(ADC_Read_Keypad_Input());
    ...
    if( currentKey != ' ')
    {
      ...
      display.setCursor((11+chSpaces1[spacesToMove]), 25);
      display.print(currentKey);
      //Now delete old ch location:
...
void Nokia_SetBatteryVoltageCutoff(void)
{
  ...
  Nokia_ClearLine(15,2,11);
  display.setCursor(2, 15);
  display.write("Set Bat V:");
  Nokia_ClearLine(25,11,11);
  display.setCursor(10, 32);
  display.write("0010 V");//0,6, 18 VV V
  ...
  display.write(25);//down arrow character

  display.setCursor(55, 27);
  display.write(65);//A character

  display.setCursor(55, 35);
  display.write(66);//B character

  display.setCursor(62, 27);
  display.write(67);//C character

  display.setCursor(62, 35);
  display.write(68);//D character
  display.display();//Update the display
  ...
  while(Done != true)
  {
    ...
    if(currentKey == 'A')//increase voltage by 1
    {
      ...
      display.setCursor(10, 32);
      display.print(ModulusOfNumber_toStringNumber(initVolts));
      display.write(" V");//0,6, 18 VV V
      display.display();//Update the display
      currentKey = ' ';
    }
...
void MasterInit(void)
{
  //this means the device is running for the very first time.
    ...
    display.setCursor(2, 15);
    display.write("Clock set!");
    display.display();//Update the display
    delay(1000);

    Nokia_ClearLine(15,2,10);
    display.setCursor(2, 15);
    display.write("Set Timer:");
    display.display();//Update the display
    delay(1000);
    ...
    display.write("All timers");
    display.setCursor(4, 22);
    display.write("are set!");
    display.display();//Update the display
    delay(1000);
...
void Master_Reset_Init(void)
{
  ...
  InitSystemTimer();//Start the millis timer for later use in timing functions
...
void OS_Run(void)
{
  ...
  switch (STATES)
  {
  case S_initsetup0://Check if it is 1st time boot
    Master_Reset_Init();
    break;
  case S_initsetup1://Setup the settings
    MasterInit();//setup all the system settings
    //update the timers:
    ReadtimerValues(1,7,13,19);
    //When the settings are done, go to main loop
    STATES = S_mainmenu;//case 2
    break;
  case S_mainmenu://Display the main menu on LCD
  ...
  case 3://switch the LCD backlight on/off
    ...
    break;
  case S_CheckVoltage://Check Voltage:
    //Serial.println("Checking bat volts");
    ...
    //STATES = S_switchContactor;
    break;
  case S_CheckTimers://Check Timers:
    //Check the main timer loop:
    Serial.print("RTC hours: " + RTC_hours + "\n" + "RTC minutes: " + RTC_minutes + "\n" + "RTC seconds: " + RTC_seconds);
    Serial.print("RTC time conversion: ");
    Serial.println(convertRTCtime(RTC_hours, RTC_minutes, RTC_seconds));
    //Serial.print("EEPROM start time conversion: ");
    //Serial.println((getTimeFromEEPROM(RoutineTimerStart,6)));
    //Serial.print("EEPROM end time conversion: ");
    ...
    test2 = (getTimeFromEEPROM(RoutineTimerEnd,6)).toInt();
    delay(10);
    test1 = (convertRTCtime(RTC_hours, RTC_minutes, RTC_seconds)).toInt();
    delay(10);
    test3 = (getTimeFromEEPROM(RoutineTimerStart,6)).toInt();
    delay(10);
    Serial.print("EEPROM Start Time: ");
    Serial.println(test3);
    Serial.print("EEPROM End Time: ");
    ...
    if( test2 <= test1 )//if start time is smaller than current time
    {
    ...
    STATES = S_switchContactor;
    break;
  case S_switchContactor://Switch Contactor ON/OFF this state here is controlled
    // inside the battery_voltage() function in case 4
    //check the status of the flag:
    Serial.println("switching contactor");
  ...
  case 8://Read any keypresses
    ...
    break;
  case S_idlescreen://IDLE STATE, Displaying time and voltage
    RTC_getInfo();//Read the time from the RTC:
    Nokia_Display_RTC_Time(12,19);//Function to display the RTC time on the LCD
    Nokia_Display_Volts(35,35);//Function to display the Battery voltage on the LCD
    STATES = S_CheckVoltage;//S_CheckTimers;//S_CheckKeypress;// case 8
    break;
  case 10:
    //
    break;
